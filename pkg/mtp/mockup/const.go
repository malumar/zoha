...
var LocalMailbox = []mtp.AddressEmail{
	mtp.SanitizeEmailAddress(LocalUser1Email),
	mtp.SanitizeEmailAddress(LocalUser2Email),
	mtp.SanitizeEmailAddress(LocalUser3Email),
	mtp.SanitizeEmailAddress(LocalUser4Email),
}

var LocalAlias = []mtp.AddressEmail{
	mtp.SanitizeEmailAddress(LocalAlias1Src),
	mtp.SanitizeEmailAddress(LocalAlias2Src),
}

var LocalUsers = append(LocalMailbox, LocalAlias...)

func IsLocalUser(lowerCaseAcsiiEmail string) bool {
	for _, v := range LocalUsers {
		if lowerCaseAcsiiEmail == v.String() {
			return true
		}
	}
